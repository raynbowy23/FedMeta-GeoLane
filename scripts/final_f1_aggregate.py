...
from strategy_annotation_eval import prf, bounds_to_centerlines_m, deployed_theta, BASELINE_THETA, SEEN
...
def main():
    ...
    for cam in SEEN + UNSEEN:
        ...
        for config in CONFIGS:
            seeds = [42] if config == 'baseline' else SEEDS  # baseline deterministic
            for seed in seeds:
                try:
    ...
    out = Path('results/final_batch_f1'); out.mkdir(parents=True, exist_ok=True)
    with open(out / 'final_batch_f1.csv', 'w', newline='') as fh:
        w = csv.DictWriter(fh, fieldnames=list(rows[0].keys())); w.writeheader(); w.writerows(rows)
    print(f'wrote {out}/final_batch_f1.csv ({len(rows)} rows)')
    ...
    for tau in TAUS:
        for split in ['seen', 'unseen']:
            ...
            for config in CONFIGS:
                ...
                seeds = [42] if config == 'baseline' else SEEDS
                for seed in seeds:
